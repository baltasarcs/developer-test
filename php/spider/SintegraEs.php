...
class SintegraEs {
	...
	function getIdentificacao($result) {
		...
		$patterns = array(	'cnpj'			=> '@<td\b[^>]*?>\s*CNPJ:</td>\s*<td\b[^>]*?>\s*(.*?)\s*</td>@is',
	...
	function getEndereco($result) {
		...
		$patterns = array(	'logradouro'	=> '@<td\b[^>]*?>\s*Logradouro:</td>\s*<td\b[^>]*?>\s*(.*?)\s*</td>@i',
	...
	function getComplemento($result) {
		...
		$patterns = array(	'atividade'				=> '@<td\b[^>]*?>\s*Atividade\s*Econ.*?mica\s*:\s*</td><td\b[^>]*?>\s*(.*?)\s*</td>@is',
